...
from plaineutils import generate_repeatable_id, invoke_chain, preseeded_questions_list
import threading
...
st.set_page_config(layout="wide", page_title="plaine.gnyan.ai",
                   initial_sidebar_state="expanded",)

scol1, scol2 = st.sidebar.columns([2, 4])
with scol1:
    scol1.image("http://192.168.27.10:8000/static/images/plaine_3.png", width=75)
with scol2:
    scol2.subheader("Welcome to Plaine!\nShip your solutions swiftly")
st.sidebar.markdown("---")


st.title("Plaine :cloud:", )
st.subheader("What do you wish to deploy today?", divider=True)
...
user_input = st.text_area("", st.session_state['user_input'], height=150)
...
with buttons:
    button_col1, button_col2, button_col3 = buttons.columns(3)
...
if folder_path:
    ...
    if return_select and return_select['checked']:
        st.subheader("Code to Cloud", divider=True)
    ...
    st.sidebar.button("Commit to Git")
